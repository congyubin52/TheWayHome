		...
		http.csrf().disable()
				...
				.authorizeHttpRequests(request -> request
						...
						.requestMatchers("/css/**", "/error/**", "/img/**", "/js/**", "", "/",
								"/user/create_account_form", "/admin/create_account_form", "/user/create_account_confirm", "/admin/create_account_confirm").permitAll()   // Security 제외
						.anyRequest().authenticated()
				)
				.formLogin(login -> login                           // 로그인 시 폼(form)을 이용
						.loginPage("/user/member_login_form")            // 로그인시 폼 주소 설정
				...
				.formLogin(login -> login
						.loginPage("/admin/member_login_form")
						...
						.permitAll())
				.logout()
				.logoutUrl("/user/member_logout_confirm")
				.logoutSuccessUrl("/")
				.logoutUrl("admin/member_logout_confirm")
				.logoutSuccessUrl("/");
		return http.build();
	}
